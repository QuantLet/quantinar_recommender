...
# Train DTM model
def train_dtm_lda_model(corpus, dictionary, time_slices, num_topics=5):
...
def train_and_visualize_lda(corpus, dictionary, num_topics=5):
    lda_model = LdaModel(corpus=corpus, id2word=dictionary, num_topics=num_topics, passes=15, random_state=42)
    vis_data = gensimvis.prepare(lda_model, corpus, dictionary)
    pyLDAvis.save_html(lda_vis, os.path.join(figure_dir, "lda_visualization.html"))
...
def main():
    ...
    lda_model = train_and_visualize_lda(corpus, dictionary, num_topics=num_topics)
